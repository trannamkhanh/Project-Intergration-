...
from config import get_sqlserver_connection, get_mysql_connection, get_erd_db_connection
...
# ============================
#  AUTH (SQL Server - ERD DB user table)
# ============================
...
def login():
    ...
    # Check password: assume stored as hash (use check_password_hash)
    # If password column is plain text, you may need to adjust
    if not check_password_hash(user["password"], password):
        # If check fails, maybe password is plain? For now return error
        return jsonify({"error": "Ten dang nhap hoac mat khau khong dung"}), 401
